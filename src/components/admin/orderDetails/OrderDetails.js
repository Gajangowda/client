...
function OrderDetails() {
  ...
  useEffect(() => {
    async function fetchOrderDetails() {
      try {
        const response = await axios.get(`https://localhost:4242/api/orders/${orderId}`);
  ...
  const handleStatusUpdate = async () => {
    ...
    try {
      setIsUpdating(true);
      await axios.put(`https://localhost:4242/api/orders/${orderId}`, { orderStatus: updatedStatus });
      const response = await axios.get(`https://localhost:4242/api/orders/${orderId}`);
      setOrder(response.data.order);
      navigate('/Orders')
