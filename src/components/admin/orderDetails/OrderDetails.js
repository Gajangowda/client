...
function OrderDetails() {
  ...
  useEffect(() => {
    async function fetchOrderDetails() {
      try {
        const response = await axios.get(`https://backend-nlz9.onrender.com/api/orders/${orderId}`);
  ...
  const handleStatusUpdate = async () => {
    ...
    try {
      setIsUpdating(true);
      await axios.put(`https://backend-nlz9.onrender.com/api/orders/${orderId}`, { orderStatus: updatedStatus });
      const response = await axios.get(`https://backend-nlz9.onrender.com/api/orders/${orderId}`);
      setOrder(response.data.order);
      navigate('/Orders')
